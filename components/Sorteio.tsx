...
function daysUntil(dateStr: string): number {
  ...
  return Math.ceil((exp.getTime() - agora.getTime()) / 86400000)
}
...
function formatDatetime(iso: string): string {
  ...
  // If no time component (midnight exactly and it came from a date-only string), omit time
  const temHora = iso.includes('T') && !(hora === '00' && min === '00')
  return temHora ? `${dia}/${mes}/${ano} às ${hora}h${min}` : `${dia}/${mes}/${ano}`
}

export default function Sorteio({ initialSorteio, initialParticipando, initialUser, initialVerified }: Props) {
  ...
  const handleParticipar = async () => {
    setErro('')
    setLoading(true)
    const res = await fetch('/api/sorteio/participar', { method: 'POST' })
    ...
    if (res.ok) {
      ...
      if (res.status === 401) {
        window.location.href = '/api/auth/discord?from=sorteio'
        return
      }
      if (res.status === 403) {
  ...
  return (
    <section id="sorteio" className="py-24 bg-br-dark">
      <div className="max-w-3xl mx-auto px-6">
        <div className="text-center mb-10">
          <h2 className="text-4xl md:text-5xl font-bold mb-3">
            <span className="text-br-yellow">SORTEIO</span>{' '}
            <span className="text-white">DA COMUNIDADE</span>
          </h2>
          <p className="text-gray-400 text-lg">Participe e concorra a prêmios exclusivos</p>
        </div>
        ...
        {!sorteio ? (
          <div className="bg-br-dark2 border border-white/10 rounded-2xl p-10 text-center">
            ...
            <p className="text-white/40 text-sm">Fique de olho no Discord! Anunciamos os sorteios por lá.</p>
          </div>
        ) : sorteio.sorteado ? (
          <div className="bg-br-dark2 border border-br-yellow/30 rounded-2xl p-8 text-center">
            ...
            <h3 className="text-lg font-bold text-white mb-1 leading-snug">{sorteio.titulo}</h3>
            {sorteio.descricao && <p className="text-white/50 text-sm mb-6 leading-relaxed whitespace-pre-wrap">{sorteio.descricao}</p>}
            {sorteio.vencedores?.length > 0 && (
              <div className="bg-br-yellow/10 border border-br-yellow/30 rounded-xl p-6">
                <p className="text-white/50 text-sm mb-3">
                  {sorteio.vencedores.length === 1 ? 'Vencedor do sorteio' : `${sorteio.vencedores.length} ganhadores do sorteio`}
                </p>
                  ...
                  {sorteio.vencedores.map(v => (
                    <div key={v.id} className="flex items-center justify-center gap-2">
                      {sorteio.modoResultado === 'colocacao' && (
                        <span className="text-2xl">{['🥇','🥈','🥉'][v.colocacao - 1] ?? `${v.colocacao}º`}</span>
                      )}
          ...
          <div className="bg-br-dark2 border border-white/10 rounded-2xl p-8">
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              {sorteio.vipBonus && (
                <div className="flex items-center gap-2 bg-br-yellow/10 border border-br-yellow/30 rounded-full px-4 py-1.5 text-sm text-br-yellow font-semibold">
                  <span>👑</span>
                  VIPs têm {sorteio.vipMultiplier}x mais chances!
                </div>
              )}
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-3 mb-6 flex items-center gap-2 text-xs text-white/40">
              <span>✅</span>
              Obrigatório ter a conta VRChat verificada para participar
            </div>

            {/* CTA */}
            {!initialUser ? (
              <a
                href="/api/auth/discord?from=sorteio"
